...
typedef RT_MODEL_QUARC_simlink_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * QUARC_simlink(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(QUARC_simlink_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://QUARC_simlink:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: QUARC_simlink -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
